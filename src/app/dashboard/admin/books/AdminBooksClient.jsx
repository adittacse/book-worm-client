...
export default function AdminBooksClient() {
    ...
    const load = async () => {
        ...
        setErr("");
        try {
            const res = await api.get("/books", { params: { page, limit, search, sort } });
            setData(res.data);
        } catch (e) {
    ...
    const loadGenres = async () => {
        try {
            ...
            const list = res.data || [];
            const map = {};
            list.forEach((g) => (map[g._id] = g.name));
            setGenreMap(map);
        } catch {
            setGenreMap({});
        }
    ...
    const openDelete = (book) => {
        ...
        document.getElementById("bw_delete_book_modal")?.showModal?.();
    };

    const closeDelete = () => {
        setDeleteId(null);
        setDeleteTitle("");
        document.getElementById("bw_delete_book_modal")?.close?.();
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (isLastItemOnPage) setParams({ page: page - 1 });
            else await load();
        } catch (e) {
        ...
        <div className="space-y-5">
            <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                    ...
                    <p className="text-sm opacity-70">Create, edit, search books and manage cover images.</p>
                </div>
                            ...
                            <table className="table">
                                ...
                                {books.map((b, index) => (
                                    <tr key={b._id}>
                                                ...
                                                <div className="avatar">
                                                    <div className="w-10 h-14 rounded">
                                                        <Image
                                                            width={40}
                                                            height={56}
                                                            src={b?.coverImage?.trim() || "https://placehold.co/600x800?text=No+Cover"}
                                                            alt={b?.title || "cover"}
                                                            className="object-cover"
                                                            // placehold SVG issue এড়াতে:
                                                            unoptimized={(b?.coverImage || "").includes("placehold.co")}
                                                        />
                                                    </div>
                                                </div>
                                        ...
                                        <td>{genreMap[b.genreId] || "Unknown"}</td>
                                        ...
                                        <td className="text-right">
                                            <div className="flex justify-end gap-2">
                                                <Link href={`/dashboard/admin/books/${b._id}/edit`} className="btn btn-sm btn-outline">
                                                    Edit
                                                </Link>
                                                <button className="btn btn-sm btn-error" onClick={() => openDelete(b)}>
                                                    Delete
                                                </button>
                                            </div>
                                        </td>
            ...
            <dialog id="bw_delete_book_modal" className="modal">
                <div className="modal-box">
                    ...
                    <p className="py-2 text-sm opacity-80">
                        Are you sure you want to delete <span className="font-semibold">{deleteTitle}</span>? This action cannot be undone.
                    </p>
